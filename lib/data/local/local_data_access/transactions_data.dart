...
class clsTransactions_data {
  ...
  static Future<String?> getTransactionIDByPaymentID(String PaymentID) async {
    List<Map> data = await SQLDB().readData(
      '''
      select id from `transactions` where paymentID = ?
       ''',
      [PaymentID],
    );
    return data == [] ? null : data[0]['id']!.toString();
  }
  ...
  static Future<int> markForDeleteByPaymentID(String paymentID) async {
    return await SQLDB().updateData(
      '''
      UPDATE `transactions` SET
        isDeleted = 1, isSynced = 0
