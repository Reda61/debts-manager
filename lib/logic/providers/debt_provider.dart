...
class clsDebtProvider extends ChangeNotifier {
  ...
  Future<void> getAllDebts() async {
    ...
    await getRecivableSum();
    await getPayablesSum();
    setDebtsTypeLists();
  }
  ...
  Future<bool> decreasePaidAmountAndRefreshDebts(double amount) async {
    bool isDebtUpdated = await clsDebts_data.decreasePaidAmount(
      _debt.id!,
      _debt.paidAmount - amount,
    );

    if (isDebtUpdated) {
      _debt.isSettled = false;
      _debt.paidAmount -= amount;
      await getAllDebts();
      notifyListeners();

      return isDebtUpdated;
    }
  ...
  Future<bool> settleThisDebt() async {
    if (_debt.isSettled) return false;
