...
class _clsDebtInfoScreenState extends State<clsDebtInfoScreen> {
  ...
  late ClsTransactionProvider transactionPro;
  ...
  Future<void> loadPaymentsByDebtID() async {
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          Consumer<clsDebtProvider>(
            builder: (context, debtpro, _) {
              return Container(
                  ...
                  children: [
                    Column(
                      children: [
                        ...
                        SizedBox(height: 10),
                        Text(
                          '${ClsUtils_Formatter.amountFormat(debtpro.debt.paidAmount)}',
                          style: Theme.of(context).textTheme.bodyMedium!
                              .copyWith(
                                color: _debt.isPaidToMe
                                    ? Colors.green
                                    : Colors.red,
                              ),
                        ),
                    ...
                    Column(
                      children: [
                        ...
                        SizedBox(height: 10),
                        Text(
                          '${ClsUtils_Formatter.amountFormat(debtpro.debt.amount - debtpro.debt.paidAmount)}',
                          style: Theme.of(context).textTheme.bodyMedium!,
                        ),
                      ],
                    ),
          ...
          Consumer<clsPaymentProvider>(
            builder: (context, paymentPro, _) {
              return Container(
                decoration: BoxDecoration(
                  color: Theme.of(context).cardColor,
                ...
                child: _paymentsOfDebt.isEmpty
                    ...
                    : ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          return Dismissible(
                            key: Key(_paymentsOfDebt[index]['id']),
                            ...
                            confirmDismiss: (direction) async {
                              ...
                              ClsAppDialog.showDeleteConfirm(
                                ...
                                onConfirm: () async {
                                  await debtPro
                                      .decreasePaidAmountAndRefreshDebts(
                                        _paymentsOfDebt[index]['amount'],
                                      );
                                  await transactionPro.setDeleteByPaymentID(
                                    _paymentsOfDebt[index]['id'],
                                  );
                                  // set Delete Payment
                                  if (await paymentPro.setDelete(
                                    _paymentsOfDebt[index]['id'],
                                  )) {
                                    isDeleted = true;
                                  }
          ...
          SizedBox(height: 7),
          Consumer<clsDebtProvider>(
            builder: (context, cDebtPro, _) {
              return Container(
                height: 120,
                padding: EdgeInsets.all(5),
                // color: Colors.amber,
                child: !cDebtPro.debt.isSettled
                    ? Column(
                        children: [
                          Expanded(
                            child: Container(
